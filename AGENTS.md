...
- **Images must be curated, not dumped.** Transfer relevant images from raw to wiki, but be selective. Include 3-5 images per article maximum. Choose images that convey information text cannot: diagrams, charts, data visualizations, before/after comparisons. Skip decorative images, duplicate illustrations of the same point, and low-information screenshots. The viewer renders images as small Wikipedia-style thumbnails floated right (max 220px wide) — too many images breaks the page layout. When in doubt, use fewer images.
